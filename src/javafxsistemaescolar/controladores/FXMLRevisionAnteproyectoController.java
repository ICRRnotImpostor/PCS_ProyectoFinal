...
Descripción: Controlador se encarga de realizar la validacion o devolver el proyecto postulado por el academico
*/
...
public class FXMLRevisionAnteproyectoController implements Initializable {
    ...
    @FXML
    private void clicBtnDevolver(ActionEvent event) {
        validarCampos();
    } 

    @FXML
    private void clicBtnValidar(ActionEvent event) {
        String nombreBoton = btnValidar.getText();
                
        if(nombreBoton == "Postular"){
            postularAnteproyecto();
        }else if(nombreBoton == "Validar"){
            validarAnteproyecto();
        }
    }
    
    public void inicializarInformacionDetalles(boolean bandera, Anteproyecto anteproyectoDetalles){
        this.anteproyectoDetalles = anteproyectoDetalles;
        this.bandera = bandera;
        cargarInformacionEdicion();
        deshabilitarBotones(bandera);
    }
    ...
    private void cargarInformacionEdicion(){
        ...
        taObservaciones.setText(anteproyectoDetalles.getObservaciones());
    ...
    private void establecerEstiloNormal(){
        taObservaciones.setStyle(estiloNormal);
    }
    ...
    private void validarCampos(){
        ...
        if(observaciones.isEmpty()){
            taObservaciones.setStyle(estiloError);
            datosValidos = false;
        }
        if(datosValidos){
            Anteproyecto ingresarObservacionesAnteproyecto = new Anteproyecto();
            ingresarObservacionesAnteproyecto.setObservaciones(observaciones);
            int idAnteproyecto = anteproyectoDetalles.getIdAnteproyecto();
            ingresarObservacionesAnteproyecto.setIdAnteproyecto(idAnteproyecto);
            devolverAnteproyecto(ingresarObservacionesAnteproyecto);
      
        }
        
    }
    
    private void devolverAnteproyecto(Anteproyecto anteproyectoObservaciones){
        int codigoRespuesta = AnteproyectoDAO.ingresarObservacionesAnteproyecto(anteproyectoObservaciones);
    ...
    private void validarAnteproyecto(){
        Anteproyecto actualizarAnteproyecto = new Anteproyecto();
        actualizarAnteproyecto.setEstado("validado");
        int idAnteproyecto = anteproyectoDetalles.getIdAnteproyecto();
        actualizarAnteproyecto.setIdAnteproyecto(idAnteproyecto);
        actualizarEstadoAnteproyecto(actualizarAnteproyecto);
    }
    
    private void postularAnteproyecto(){
        Anteproyecto actualizarAnteproyecto = new Anteproyecto();
        actualizarAnteproyecto.setEstado("pendiente");
        int idAnteproyecto = anteproyectoDetalles.getIdAnteproyecto();
        actualizarAnteproyecto.setIdAnteproyecto(idAnteproyecto);
        actualizarEstadoAnteproyecto(actualizarAnteproyecto);
    }
    
    private void actualizarEstadoAnteproyecto(Anteproyecto anteproyectoValidado){
        int codigoRespuesta = AnteproyectoDAO.validarAnteproyecto(anteproyectoValidado);
        switch(codigoRespuesta){
            case Constantes.ERROR_CONEXION:
                Utilidades.mostrarDialogoSimple("Error de conexión", 
                        "El Anteproyecto no pudo ser actualizado debido a un error en su conexión...", 
    ...
    private void deshabilitarBotones(boolean bandera){
        btnValidar.setText("Postular");
        btnDevolver.setDisable(bandera);
